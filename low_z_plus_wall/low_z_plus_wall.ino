  ...
  switch (STATE) {
    ...
    case STATE_NORMAL_CONTROL: // General GO state
      ...
      if (apparent_speed > max_speed || apparent_speed < -max_speed) {
        ...
        reaction = -(long)(stiffness * error); // Re-implement forecasting later
      } else {
        digitalWrite(DEBUG_LED_PIN, LOW);
        input = (sgread + lastsgread) / 2;
        target = sgoffset - sg_calib_offset + (long)(GRAVITY_STRAIN * sin(PI * (encread - CALIB_MID) / 2048.0f)); // no gravity compensation yet
