...
const int SG_OFFSET_AT_CALIBRATION = 47; // SG at calib minus streaming SG reading
...
const float GRAVITY_STRAIN = -70.0f;
...
void loop() {
  switch (STATE) {
    ...
    case STATE_NORMAL_CONTROL: // General GO state
      ...
      //Serial.println(csread);
      //delay(100); // TAKE THIS OUT LATER!!!!!!!!!
      checkSerial();
...
void controllIt()
{
  ...
  else
  {
    if(myPID->compute(micros())) {
      //osmc->setEffort( (input - target) * 10);
