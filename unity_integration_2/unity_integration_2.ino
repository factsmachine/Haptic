...
// Maximum motor effort (out of 65535). Never exceed 60000.
const int MAX_EFFORT = 35000;

// Constants concerning "Haptic Rendering" approach
...
// Serial communicaton characters
...
void loop() {  
  ...
  if (encread > HIGHPOS) {
    ...
    // CONTROL ALGORITHM: "HAPTIC RENDERING" APPROACH
  ...
  if (Serial.available())
  {
    ...
    {
      left_to_read--;
      temp_u_position += (serialChar << (8 * (1 - left_to_read))); // Only works for two bytes
      if (left_to_read <= 0) {
